...
# Separate fixed namespace for adversary-profile ids, same idempotency rationale as above.
ADVERSARY_ID_NAMESPACE = uuid.UUID("a3d8e1f2-9b6c-4d7a-8e1f-2c3b4a5d6e7f")
...
def write_adversary(tactic: str, ability_ids: list, out_dir: Path, dry_run: bool):
    adversary_id = str(uuid.uuid5(ADVERSARY_ID_NAMESPACE, f"adversary:{tactic}"))
    label = tactic.replace("-", " ").title()
    adversary = {
        "name": f"{NAME_PREFIX}{label}",
        "description": (
            f"Every Atomic Red Team ability generated by this repo's generator for the "
            f"'{tactic}' tactic, run in the order they were generated."
        ),
    ...
    dest = out_dir / "adversaries" / f"{tactic}.yml"
    if dry_run:
        print(f"--- {dest.relative_to(out_dir.parent) if dest.is_relative_to(out_dir.parent) else dest} ---")
        print(text)
        return
...
def main():
    ...
    for tech_dir in tech_dirs:
        ...
        for test in doc["atomic_tests"]:
            ...
            # Only abilities that are clean (mapped tactic, no missing payload) go into
            # an adversary profile - those are the ones actually safe to run as-is.
            if not unmapped and not has_missing_payload:
                ability_ids_by_tactic.setdefault(ability["tactic"], []).append(ability["id"])

    for tactic, ability_ids in sorted(ability_ids_by_tactic.items()):
        write_adversary(tactic, ability_ids, out_dir, args.dry_run)
    ...
    if not args.dry_run:
        ...
        print(f"Generated {len(ability_ids_by_tactic)} adversary profiles into {out_dir / 'adversaries'}")
